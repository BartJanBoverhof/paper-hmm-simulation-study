...
theta.mod.overlap <-matrix(c(0.59, 0.29, 0.04, 0.04, 0.04,
                             0.04, 0.29, 0.59, 0.04, 0.04,
                             0.04, 0.04, 0.20, 0.36, 0.36), byrow = TRUE, nrow = m)
...
for(sim_i in 1:n_sim){
  data1[[sim_i]] <- sim_mHMM(n_t = n_t, n = n, m = m, q_emiss = q_emiss,
                             gamma = gamma, emiss_distr = theta.mod.overlap) 
}
