...
theta.no.overlap <- matrix(c(0.84, 0.04, 0.04, 0.04, 0.04,
                             0.04, 0.44, 0.44, 0.04, 0.04,
                             0.04, 0.04, 0.04, 0.44, 0.44), byrow = TRUE, nrow = m)
...
for(sim_i in 1:n_sim){
  data1[[sim_i]] <- sim_mHMM(n_t = n_t, n = n, m = m, q_emiss = q_emiss,
                             gamma = gamma, emiss_distr = theta.no.overlap) 
}
