...
theta.modclear <- matrix(c(0.68, 0.08, 0.08, 0.08, 0.08,
                           0.08, 0.38, 0.08, 0.38, 0.08,
                           0.08, 0.08, 0.38, 0.08, 0.38), byrow = TRUE, nrow = m)
...
for(sim_i in 1:n_sim){
  data1[[sim_i]] <- sim_mHMM(n_t = n_t, n = n, m = m, q_emiss = q_emiss,
                             gamma = gamma, emiss_distr = list(theta.modclear)) 
}
