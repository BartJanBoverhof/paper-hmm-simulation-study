...
for (i in 1 : (length(length_strings) - 1) ){
  
assign(paste0("bias_theta_muchoverl_obs_5_t_", length_strings[i + 1]), 
       sweep(eval(parse(text = paste0("results$sim_HMM_theta_muchoverl_obs_5_t_", length_strings[i + 1], "$out_sim$emiss_mean"))), 
             2, much_overlap_true)) 

assign(paste0("bias_theta_muchoverl_obs_5_t_", length_strings[i + 1]), 
       melt(eval(parse(text = paste0("bias_theta_muchoverl_obs_5_t_", length_strings[i + 1])))))

assign(paste0("bias_theta_muchoverl_obs_5_t_", length_strings[i + 1]), 
       cbind(eval(parse(text = paste0("bias_theta_muchoverl_obs_5_t_", length_strings[i + 1]))), 
             Length = length_strings[i + 1], Overlap = "Much"))
...
assign(paste0("sd_theta_muchoverl_obs_5_t_", length_strings[i + 1]), 
       melt(eval(parse(text = paste0("results$sim_HMM_theta_muchoverl_obs_5_t_", length_strings[i + 1], "$out_sim$emiss_sd")))))

assign(paste0("sd_theta_muchoverl_obs_5_t_", length_strings[i + 1]), 
       cbind(eval(parse(text = paste0("sd_theta_muchoverl_obs_5_t_", length_strings[i + 1]))), 
             Length = length_strings[i + 1], Overlap = "Much"))
...
# Much overlap. 
sd_muchoverl_obs_5_theta <- rbind(data.frame(sd_theta_muchoverl_obs_5_t_500), data.frame(sd_theta_muchoverl_obs_5_t_1000), 
                                  data.frame(sd_theta_muchoverl_obs_5_t_2000), data.frame(sd_theta_muchoverl_obs_5_t_4000), 
                                  data.frame(sd_theta_muchoverl_obs_5_t_8000))
...
for (i in 1 : (length(length_strings) - 1) ){
  ...
  assign(paste0("bias_gamma_muchoverl_obs_5_t_", length_strings[i + 1]), 
         sweep(eval(parse(text = paste0("results$sim_HMM_theta_muchoverl_obs_5_t_", length_strings[i + 1], "$out_sim$gamma_mean"))), 
               2, gamma_true)) 
  
  assign(paste0("bias_gamma_muchoverl_obs_5_t_", length_strings[i + 1]), 
         melt(eval(parse(text = paste0("bias_gamma_muchoverl_obs_5_t_", length_strings[i + 1])))))
  
  assign(paste0("bias_gamma_muchoverl_obs_5_t_", length_strings[i + 1]), 
         cbind(eval(parse(text = paste0("bias_gamma_muchoverl_obs_5_t_", length_strings[i + 1]))), 
               Length = length_strings[i + 1], Overlap = "Much"))
...
assign(paste0("sd_gamma_muchoverl_obs_5_t_", length_strings[i + 1]), 
       melt(eval(parse(text = paste0("results$sim_HMM_theta_muchoverl_obs_5_t_", length_strings[i + 1], "$out_sim$gamma_sd")))))

assign(paste0("sd_gamma_muchoverl_obs_5_t_", length_strings[i + 1]), 
       cbind(eval(parse(text = paste0("sd_gamma_muchoverl_obs_5_t_", length_strings[i + 1]))), 
             Length = length_strings[i + 1], Overlap = "Much"))
