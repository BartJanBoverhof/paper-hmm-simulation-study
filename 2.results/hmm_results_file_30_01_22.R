  ...
  if (varobs == T){
    ...
    cov_complete_plot <- cov %>%
      ...
      cov <- cov %>%
        ...
        mutate(length = factor(length, levels = c("250", "500", "1000", "2000", "4000", "8000")),
               cats = factor(cats, levels = c(paste0("M_S1_cat", ord), paste0("M_S2_cat", ord), paste0("M_S3_cat", ord)))) 
      temp <- cbind(expand.grid(levels(cov$cats), levels(cov$condition)), length(unlist(true_probs)))
...
clarity_theta <- calcR_theta(true_probs = true_probs, nobs = nobs, cond = cond, condition = condition, cond_string = cond_string)
...
## Absolute bias. 
# Reduced plot. 
overlap_theta$abs_bias$abs_bias <- overlap_theta$abs_bias$abs_bias %>% 
  mutate(cats_n = case_when(cats == "M_S1_cat1" & condition == "None" ~ "One-indicator state", 
                            cats == "M_S1_cat1" | cats == "M_S1_cat2"  & condition == "Much" ~ "Two-indicator state",
                            cats == "M_S2_cat2" | cats == "M_S2_cat3"  & condition == "None" | condition == "Much" ~ "Two-indicator state",
                            cats == "M_S3_cat4" | cats == "M_S3_cat5"  & condition == "None" ~ "Two-indicator state",
                            cats == "M_S3_cat3" | cats == "M_S3_cat4" | cats == "M_S3_cat5"  & condition == "Much" ~ "Three-indicator state", 
                            cats == "M_S1_cat1" | cats == "M_S2_cat3" & condition == "Moderate" ~ "Two-indicator primary state",
                            cats == "M_S1_cat2" | cats == "M_S2_cat2" & condition == "Moderate" ~ "Two-indicator secondary state", 
                            cats == "M_S3_cat4" | cats == "M_S3_cat5" & condition == "Moderate" ~ "Three-indicator primary state",
                            cats == "M_S3_cat3" & condition == "Moderate" ~ "Three-indicator secondary state"))
overlap_theta$abs_bias$abs_bias <- overlap_theta$abs_bias$abs_bias %>% mutate(cats_n = replace_na(cats_n, "Noise"))
...
## Relative bias. 
# Reduced plot. 
overlap_theta$rel_bias$rel_bias <- overlap_theta$rel_bias$rel_bias %>% 
  mutate(cats_n = case_when(cats == "M_S1_cat1" & condition == "None" ~ "One-indicator state", 
                            cats == "M_S1_cat1" | cats == "M_S1_cat2"  & condition == "Much" ~ "Two-indicator state",
                            cats == "M_S2_cat2" | cats == "M_S2_cat3"  & condition == "None" | condition == "Much" ~ "Two-indicator state",
                            cats == "M_S3_cat4" | cats == "M_S3_cat5"  & condition == "None" ~ "Two-indicator state",
                            ...
                            cats == "M_S3_cat4" | cats == "M_S3_cat5" & condition == "Moderate" ~ "Three-indicator primary state",
                            cats == "M_S3_cat3" & condition == "Moderate" ~ "Three-indicator secondary state"))
overlap_theta$rel_bias$rel_bias <- overlap_theta$rel_bias$rel_bias %>% mutate(cats_n = replace_na(cats_n, "Noise"))
...
## Standard deviation. 
# Reduced plot. 
overlap_theta$sdev$sdev <- overlap_theta$sdev$sdev %>% 
  mutate(cats_n = case_when(cats == "sd_S1_cat1" & condition == "None" ~ "One-indicator state", 
                            cats == "sd_S1_cat1" | cats == "sd_S1_cat2"  & condition == "Much" ~ "Two-indicator state",
                            cats == "sd_S2_cat2" | cats == "sd_S2_cat3"  & condition == "None" | condition == "Much" ~ "Two-indicator state",
                            cats == "sd_S3_cat4" | cats == "sd_S3_cat5"  & condition == "None" ~ "Two-indicator state",
                            cats == "sd_S3_cat3" | cats == "sd_S3_cat4" | cats == "sd_S3_cat5"  & condition == "Much" ~ "Three-indicator state", 
                            cats == "sd_S1_cat1" | cats == "sd_S2_cat3" & condition == "Moderate" ~ "Two-indicator primary state",
                            cats == "sd_S1_cat2" | cats == "sd_S2_cat2" & condition == "Moderate" ~ "Two-indicator secondary state", 
                            cats == "sd_S3_cat4" | cats == "sd_S3_cat5" & condition == "Moderate" ~ "Three-indicator primary state",
                            cats == "sd_S3_cat3" & condition == "Moderate" ~ "Three-indicator secondary state"))
overlap_theta$sdev$sdev <- overlap_theta$sdev$sdev %>% mutate(cats_n = replace_na(cats_n, "Noise"))
...
## Coverage 
# Reduced plot. 
overlap_theta$cov$cov <- overlap_theta$cov$cov %>% 
  mutate(cats_n = case_when(cats == "M_S1_cat1" & condition == "None" ~ "One-indicator state", 
                            cats == "M_S1_cat1" | cats == "M_S1_cat2"  & condition == "Much" ~ "Two-indicator state",
                            cats == "M_S2_cat2" | cats == "M_S2_cat3"  & condition == "None" | condition == "Much" ~ "Two-indicator state",
                            cats == "M_S3_cat4" | cats == "M_S3_cat5"  & condition == "None" ~ "Two-indicator state",
                            ...
                            cats == "M_S3_cat4" | cats == "M_S3_cat5" & condition == "Moderate" ~ "Three-indicator primary state",
                            cats == "M_S3_cat3" & condition == "Moderate" ~ "Three-indicator secondary state"))
overlap_theta$cov$cov <- overlap_theta$cov$cov %>% mutate(cats_n = replace_na(cats_n, "Noise"))
...
#####################################################################################################################################################################
### Absolute bias, relative bias, and standard error of the emission and transition probabilities number of observations unclear emission probabilities condition ###
#####################################################################################################################################################################

### Calculate absolute bias, relative bias, standard deviation, and coverage of the emission probabilities for the number 
...
## Absolute bias. 
# Reduced plot. 
nobs_uncl_theta$abs_bias$abs_bias$cats <- recode(nobs_uncl_theta$abs_bias$abs_bias$cats, 
                                               "c('M_S1_cat1') = 'One-indicator state'; 
                                                c('M_S2_cat2', 'M_S2_cat4', 'M_S3_cat3', 'M_S3_cat5') = 'Two-indicator state';
                                                else = 'Noise'")
nobs_uncl_theta$abs_bias$abs_bias$cats <- factor(clarity_theta$abs_bias$abs_bias$cats, 
                                               levels = c("One-indicator state", "Two-indicator state", "Noise"))
abs_bias_reduced_plot <- clarity_theta$abs_bias$abs_bias %>%
  group_by(cats, length, condition) %>%
  dplyr::summarize(mean = mean(mean_abs_bias), sd = sd(mean_abs_bias)) %>% 
  ggplot(aes(x = length, y = mean, color = condition, group = condition)) +
  facet_wrap(facets = vars(cats)) +
...
## Relative bias. 
# Reduced plot. 
nobs_uncl_theta$abs_bias$abs_bias$cats <- recode(clarity_theta$abs_bias$abs_bias$cats, 
                                               "c('M_S1_cat1') = 'One-indicator state'; 
                                                c('M_S2_cat2', 'M_S2_cat4', 'M_S3_cat3', 'M_S3_cat5') = 'Two-indicator state';
                                                else = 'Noise'")
clarity_theta$abs_bias$abs_bias$cats <- factor(clarity_theta$abs_bias$abs_bias$cats, 
...
### Calculate absolute bias, relative bias, standard deviation, and coverage of the transition probabilities for the number 
### of observations condition with unclear emission probabilities, with associated plots. 
...
cond_string <- c("Number of observations")
## Calculate the various measures and the associated plots. 
nobs_uncl_gamma <- calcR_gamma(true_probs = gamma_true, nobs = nobs, cond = cond, condition = condition, cond_string = cond_string)
...
###################################################################################################################################################################
### Absolute bias, relative bias, and standard error of the emission and transition probabilities number of observations clear emission probabilities condition ###
###################################################################################################################################################################

### Calculate absolute bias, relative bias, standard deviation, and coverage of the emission probabilities for the number 
### of observations condition with clear emission probabilities, with associated plots.
...
### Reduced and combined plot of the absolute bias, relative bias, standard deviation, and coverage of the emission probabilities for the number 
### of observations condition with clear emission probabilities, with associated plots.
{
## Absolute bias. 
# Reduced plot. 
overlap_theta$abs_bias$abs_bias$cats <- recode(clarity_theta$abs_bias$abs_bias$cats, 
                                               "c('M_S1_cat1') = 'One-indicator state'; 
                                                c('M_S2_cat2', 'M_S2_cat4', 'M_S3_cat3', 'M_S3_cat5') = 'Two-indicator state';
                                                else = 'Noise'")
clarity_theta$abs_bias$abs_bias$cats <- factor(clarity_theta$abs_bias$abs_bias$cats, 
